<script lang="ts">
	...
	function drawPixel(pixel: Pixel, native=true ) {
		// console.log(x, y)
		// if (prevPoint && pixel.x === prevPoint.x && pixel.y === prevPoint.y) return;
	...
	function clearPixel(point: Point, firstColor = '#545454', secondColor = '#4C4C4C') {
		const halfPixelSize = pixelSize / 2;
		for (let x = point.x; x < point.x + brushSize; x++) {
			for (let y = point.y; y < point.y + brushSize; y++) {
				ctx.fillStyle = firstColor;
				ctx.fillRect(x * pixelSize, y * pixelSize, halfPixelSize, halfPixelSize);
				ctx.fillRect(
				...
				ctx.fillRect(x * pixelSize + halfPixelSize, y * pixelSize, halfPixelSize, halfPixelSize);
				ctx.fillRect(x * pixelSize, y * pixelSize + halfPixelSize, halfPixelSize, halfPixelSize);
			}
		}
